...
def display_frames(original_frame, aligned_frame, labels):
    ...
    cv2.putText(orig, labels.get("original", "original"), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 6)
    cv2.putText(aligned, labels.get("stabilized", "stabilized"), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 6)
...
def main():
    ...
    video_path = os.path.join(ROOT_DIR, config_data['video_dir_relative'], config_data['video_name'])
    ...
    gftt = config["good_features_to_track"]
    first_gray = cv2.cvtColor(first_frame, cv2.COLOR_BGR2GRAY)
    first_pts = cv2.goodFeaturesToTrack(
        first_gray,
        maxCorners=gftt["max_corners"],
        qualityLevel=gftt["quality_level"],
        minDistance=gftt["min_distance"]
    )
    ...
    stabilizer = VideoStabilizer(
        ...
        config.get("correction_boost", 1.0),
        config.get("angle_smoothing", True),
    ...
    out_writer = cv2.VideoWriter(output_path, fourcc, config.get("output_fps", 30.0), (width, height))
    ...
    while True:
        ...
        stabilized_frame = stabilizer.stabilize(
            frame,
            config["bidirectional_max_error"],
